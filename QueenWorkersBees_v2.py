...
def queen_workerbees(prod,  queens_chess_piece='bees_manager'):
    ...
    def Return_Init_ChartData(ticker_list, chart_times): #Iniaite Ticker Charts with Indicator Data
        ...
        try:
            ...
            s = datetime.datetime.now(est)
            dfs_index_tickers = {}
            bars = return_bars_list(api=api, ticker_list=ticker_list, chart_times=chart_times, trading_days_df=trading_days_df)      
    ...
    def Return_Bars_LatestDayRebuild(ticker_time): #Iniaite Ticker Charts with Indicator Data
        ...
        try:
            ...
            bars_data = return_bars(api=api, symbol=ticker, timeframe=timeframe, ndays=0, trading_days_df=trading_days_df) # return [True, symbol_data, market_hours_data, after_hours_data]
    ...
    def queens_court__WorkerBees():
        try:
            pq = read_QUEEN(queen_db=queen_db, qcp_s=['castle', 'bishop', 'knight']) # castle, bishop
