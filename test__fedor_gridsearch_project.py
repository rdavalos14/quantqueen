...
for fast_val in fast_vals:
    for slow_val in slow_vals:
        for smooth_val in smooth_vals:
            ...
            # return all STORY_bees from macd group (all tickers)
            backtest_analyzewave_results = {}
            STORY_bee = {}
            ...
            for filepath in os.listdir(folder):
                if pattern.match(filepath):
                    ...
                    data = ReadPickleData(os.path.join(folder, filepath))
                    sb_temp = data['STORY_bee']
                    STORY_bee[ttf] = sb_temp
                    # ipdb.set_trace()
                    res = analyze_waves(STORY_bee=STORY_bee, ttframe_wave_trigbee = False)
                    res_data = res["d_agg_view_return"]

                    buycross = res_data[ttf]['buy_cross-0'] # 'sell_cross-0'
                    maxprofit = buycross["sum_maxprofit"]
                    winners = buycross["winners_n"]
                    losers = buycross["losers_n"]
                    win_ratio = winners/(winners+losers)
                    avg_length = buycross['avg_length']
                    avg_time_to_max_profit = buycross['avg_time_to_max_profit']
