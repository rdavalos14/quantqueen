...
import argparse
from chess_piece.king import hash_string, kingdom__global_vars, print_line_of_error, master_swarm_KING, return_QUEENs__symbols_data, kingdom__grace_to_find_a_Queen, ReadPickleData, PickleData
from chess_piece.queen_hive import (wave_buy__var_items,
                                    init_charlie_bee, 
                                    init_queenbee, 
                                    power_amo, 
                                    ...
                                    return_alpaca_user_apiKeys, send_email, return_STORYbee_trigbees, init_logging, convert_to_float, 
                                    order_vars__queen_order_items, init_pollen_dbs, story_view, logging_log_message, 
                                    return_alpc_portolio, return_market_hours, pollen_themes, check_order_status,  timestamp_string, 
...
def close_day__queen(QUEEN):
    # dump all completed into FINAL orders bucket # WORKERBEE
        ...
        if queen_order['queen_order_state'] in RUNNING_Orders:
            ...
            if len(closing_dfs) > 0: # WORKERBEE num convert NOT necesary REMOVE
                ...
                if sum(closing_dfs['filled_qty']) > float(queen_order['qty']):
                    msg=("wtf closing > origin order", queen_order['client_order_id'])
                    print(msg)
                    logging.error(msg)
                    pass
...
def queenbee(client_user, prod, queens_chess_piece='queen'):
    ...
    def king_knights_requests(QUEEN, STORY_bee, revrec, tm_trig, trigbee, ticker, ticker_time_frame, trading_model, trig_action, app_trig, crypto=False):
        ...
        try:
            ...
            symbol = find_symbol(QUEEN, ticker, trading_model, trigbee).get('ticker') # for SPY & QQQ to get inverse X of trading model
            star_time = f'{tframe}{"_"}{tperiod}'

            alloc_deploy = revrec['waveview'].at[ticker_time_frame, 'allocation_deploy']
            ...
            wave_amo = alloc_deploy
            # wave amo tiering
            # wave_amo, king_order_rules = knights_wave_amo(alloc_deploy, trigbee, wave_amo, ticker_time_frame, king_order_rules, star_total_budget_remaining, symbol_total_budget_remaining)
            order_side = 'buy'
                ...
                for ticker_time_frame in bees_fly.index.tolist():
                    ...
                    try: # """ Trigger Bees"""                         
                        ...
                        if revrec['df_stars'].loc[ticker_time_frame].get("remaining_budget") <= 0 and revrec['df_stars'].loc[ticker_time_frame].get("remaining_budget_borrow") <= 0:
                            ...
                            print(f'{ticker_time_frame} No Budget Remaining')
                            continue
            ...
            def waterfall_sellout_chain(client_order_id, macd_tier, trading_model, sell_order, run_order, order_type, limit_price, sell_trigbee_trigger, stagger_profits, scalp_profits, run_order_wave_changed, sell_qty, QUEEN=QUEEN):
                ...
                try:
                    ...
                    if sell_order:
                        ...
                        if sell_qty > 0:
                            msg = ("Bishop SELL ORDER:", ticker_time_frame, sell_reasons, current_macd, current_macd_time, mm_cost)
                            print(msg)
                            logging.info(msg)
