...
def queenbee(client_user, prod, queens_chess_piece='queen'):
    ...
    def king_knights_requests(QUEEN, STORY_bee, revrec, tm_trig, trigbee, ticker, ticker_time_frame, trading_model, trig_action, app_trig, crypto=False):
        ...
        try:
            ...
            def knights_wave_amo(alloc_deploy, trigbee, wave_amo, ticker_time_frame, king_order_rules, star_total_budget_remaining, symbol_total_budget_remaining):
        ...
        def check_revrec(sell_qty, revrec, trigname, current_macd, mm_cost, ticker_time_frame):
            try:
                ...
                if ticker_time_frame in revrec['waveview'].index:
                    al = revrec['waveview'].loc[ticker_time_frame].get('total_allocation_budget')
                    cb = mm_cost
                    al_sellable = al -  cb # budget 8, asking to sell 50
